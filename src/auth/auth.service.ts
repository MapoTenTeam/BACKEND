...
export class AuthService {
  ...
  async getEnterpriseProfile(@Req() req) {
    ...
    const [user] = await conn.query(
      ...
      IFNULL((SELECT CODE_NM FROM COMTCCMMNDETAILCODE WHERE CODE_ID ='COM026' AND CODE='${find.ENTRPRS_SE_CODE}'),'') AS ENTRPRS_SE,
      CMPNY_NM, BIZRNO, 
      IFNULL(CEO,'') AS CEO, 
      IFNULL(ADRES,'') AS ADRES, 
      ...
      BSNNM_APRVL_CODE,
      (SELECT CODE_NM FROM COMTCCMMNDETAILCODE WHERE CODE_ID ='crnst' AND CODE='${find.BSNNM_APRVL_CODE}') AS BSNNM_APRVL_NAME
